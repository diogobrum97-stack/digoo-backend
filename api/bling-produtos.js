...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.query.action === 'extrair-pdf-full' && req.method === 'POST') {
      ...
      // Converte PDF em texto via Claude e extrai SKUs com regex
      // O Claude apenas extrai o texto — o parse é feito aqui para evitar
      // problemas de quebra de linha no PDF que causam SKUs cortados
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: 'text', text: 'Retorna o texto completo extraído deste PDF, sem modificações, sem resumos, sem formatação extra. Apenas o texto bruto do documento.' }
            ]
          }]
        })
      });
      ...
      if (!claudeResp.ok) {
        ...
        return res.status(500).json({ erro: 'Erro Claude API: ' + err.slice(0, 200) });
      }

      const claudeData = await claudeResp.json();
      const textoRaw = claudeData.content?.[0]?.text || '';

      // Junta linhas quebradas: se linha termina com hífen e próxima começa com letras/números,
      // é uma quebra de SKU — junta sem espaço
      const textoJuntado = textoRaw.replace(/([A-Z0-9])-\n([A-Z0-9])/g, '$1-$2');

      // Extrai pares SKU → quantidade usando regex
      // Formato no PDF: "SKU: WC-HDC-PRETO" seguido de número na coluna UNIDADES
      const itens = [];
      const skuRegex = /SKU:\s*([A-Z0-9][A-Z0-9\-]+)/g;
      let match;
      const linhas = textoJuntado.split('\n');

      for (let i = 0; i < linhas.length; i++) {
        const linha = linhas[i];
        const skuMatch = linha.match(/SKU:\s*([A-Z0-9][A-Z0-9\-]+)/);
        if (!skuMatch) continue;

        const sku = skuMatch[1].trim();

        // Busca a descrição — linha anterior geralmente tem o nome do produto
        let descricao = '';
        for (let j = i - 1; j >= Math.max(0, i - 4); j--) {
          const l = linhas[j].trim();
          if (l.length > 10 && !l.match(/^[0-9]+$/) && !l.includes('Código') && !l.includes('SKU')) {
            descricao = l;
            break;
          }
        }

        // Busca a quantidade — número isolado nas próximas linhas
        let quantidade = 0;
        for (let j = i + 1; j < Math.min(linhas.length, i + 8); j++) {
          const qMatch = linhas[j].trim().match(/^(\d+)$/);
          if (qMatch) {
            quantidade = parseInt(qMatch[1]);
            break;
          }
        }
        ...
        if (sku && quantidade > 0) {
          // Evita duplicatas
          if (!itens.find(it => it.sku === sku)) {
            itens.push({ sku, descricao: descricao || sku, quantidade });
          }
        }
      }

      if (!itens.length) {
        return res.status(500).json({ erro: 'Nenhum SKU encontrado no PDF. Verifique o formato do arquivo.', textoRaw: textoRaw.slice(0, 500) });
      }
